...
def menu_():
    btn = [
        ...
        types.InlineKeyboardButton(text='Criteria', callback_data='criteria')
    ]
...
def popular_movie_buttons(first, popular_list, original_name, id):
    buttons = []
    buttons.append(types.InlineKeyboardButton(text='Trailer YouTube',
                                              url=f'https://www.youtube.com/results?search_query=+{original_name}+ trailer'))

    buttons.append(types.InlineKeyboardButton(text="More Info On TMDB", url=f'https://www.themoviedb.org/movie/{id}'))
    if not first <= 0:
...
def title_movie_buttons(first, movie_list):
    buttons = []
    if not first <= 0:
...
def total_keyboard():
    buttons = []
    buttons.append(types.InlineKeyboardButton(text='total', callback_data='total_0'))
    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(*buttons)
    return keyboard
